...
from demo.product.models import Product
...
@strawberry.type
class Query:
    orders_conn: ListConnectionWithTotalCount[OrderType] = strawberry_django.connection(
        extensions=[IsStaff()],
    )

    @strawberry_django.connection(
        ListConnectionWithTotalCount[OrderType],
...
class Mutation:
    ...
    def cart_remove_item(
        ...
        # Save pk again to return the removed object after as django will set it to None
        pk = cart_item.pk
        cart_item.delete()
        cart_item.pk = pk
